...
async function init(){
  ...
  stationsEl.innerHTML=stationList.map(s=>`<button onclick="station='${s}';document.querySelectorAll('#stations button').forEach(b=>{b.classList.remove('bg-slate-900','text-white');b.style.cssText='background:rgba(255,255,255,0.60);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border:2px solid rgba(255,255,255,0.80);box-shadow:0 2px 8px rgba(30,58,95,0.08),inset 0 1px 0 rgba(255,255,255,0.9);';});this.classList.add('bg-slate-900','text-white');this.style.cssText='border:2px solid #1e293b;';feedMode?renderFeed():renderM()" style="${station===s?'border:2px solid #1e293b;':'background:rgba(255,255,255,0.60);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border:2px solid rgba(255,255,255,0.80);box-shadow:0 2px 8px rgba(30,58,95,0.08),inset 0 1px 0 rgba(255,255,255,0.9);'}" class="px-3 py-1 rounded-full text-sm ${station===s?'bg-slate-900 text-white':''}">${s.replace(" Station","").replace("Chiusura","EOD")}</button>`).join('');
  // Tab closing: stazioni dinamiche dal DB (solo quelle con item attivi)
  const closingStations = ['All', ...Array.from(new Set(closingItems.map(i=>i.station))).sort()];
  if(!closingStations.includes(station2)) station2 = closingStations[1] || 'All';
  stations2El.innerHTML=closingStations.map(s=>`<button onclick="station2='${s}';document.querySelectorAll('#stations2 button').forEach(b=>{b.classList.remove('bg-slate-900','text-white');b.style.cssText='background:rgba(255,255,255,0.60);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border:2px solid rgba(255,255,255,0.80);box-shadow:0 2px 8px rgba(30,58,95,0.08),inset 0 1px 0 rgba(255,255,255,0.9);';});this.classList.add('bg-slate-900','text-white');this.style.cssText='border:2px solid #1e293b;';renderS()" style="${station2===s?'border:2px solid #1e293b;':'background:rgba(255,255,255,0.60);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border:2px solid rgba(255,255,255,0.80);box-shadow:0 2px 8px rgba(30,58,95,0.08),inset 0 1px 0 rgba(255,255,255,0.9);'}" class="px-3 py-1 rounded-full text-sm ${station2===s?'bg-slate-900 text-white':''}">${s.replace(" Station","").replace("Chiusura","EOD")}</button>`).join('');
